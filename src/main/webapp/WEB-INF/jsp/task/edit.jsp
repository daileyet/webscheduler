				...
				<form action="${ew:path('/task/edit')}" method="post"
						...
						<div class="col-sm-10">
							<select id="tasktype" name="tasktype" class="form-control" required>
								...
								<c:forEach var="typeClass" items="${types }">
									<option <c:if test="${pm.tm.taskType == typeClass.name }">selected</c:if> value="${typeClass.name }" >
					...
					<div class="form-group">
						<label for="taskname" class="col-sm-2 control-label">Task
							Properties</label>
						<div class="col-sm-10">
							<textarea class="form-control" id="tasktref" name="taskref" rows="10">${pm.tm.taskRefContent}</textarea>
						</div>
					</div>
					<div class="form-group">
